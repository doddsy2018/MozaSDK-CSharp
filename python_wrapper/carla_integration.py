...
class DualControl(object):
    ...
    def _parse_vehicle_wheel(self):
        global moza_steer, moza_brake, moza_throttle
        ...
        '''
        K=0.1
        steerCmd=K*math.radians(moza_steering)
        brakeCmd=moza_brake/100
        throttleCmd=moza_throttle/100
...
def game_loop(args):
    ...
    try:
        ...
        window_title = "Carla Moza"
        pygame.display.set_caption(window_title)
        # Turn on spring force
        moza.pySetSpringForce(window_title)
        ...
        moza.pySetSpringForce(window_title,1000)
...
if __name__ == '__main__':
    ...
    def update_controls():
        global moza_throttle, moza_brake, moza_steering
        while True:
            HIDData= moza.pygetHIDData()
            moza_steering=round(HIDData["steeringangle"],2)
            moza_throttle=round(HIDData["throttle"],2)
            moza_brake=round(HIDData["brake"],2)
            #print (round(HIDData["steeringangle"],2), round(HIDData["throttle"],2), round(HIDData["brake"],2))
    def upate_switches():
        global moza_currentSwitchValue
        while True:
            moza_currentSwitchValue=moza.pyGetSwitchValues()
            print ("currentSwitchValue ", moza_currentSwitchValue, " is ON")

    # Initialize control variables
    moza_throttle=0
    moza_brake=0
    moza_steering=0
    moza_currentSwitchValue=0
